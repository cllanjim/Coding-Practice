...
//GroupView *example = [[[NSBundle mainBundle] loadNibNamed:@"GroupView" owner:nil options:nil] firstObject];
   NSArray *total = [[NSBundle mainBundle] loadNibNamed:@"GroupView" owner:nil options:nil];
    UIView *a = total[0];
     GroupView *b =[[a subviews] objectAtIndex:1];
    
    //NSLog(@"%ld",total.count);
    //GroupView *example  = a[0];
...
- (IBAction)clickIconButton:(UIButton *)iconButton
{
    ...
    labelOfLoading.text = self.iconLabel.text;
