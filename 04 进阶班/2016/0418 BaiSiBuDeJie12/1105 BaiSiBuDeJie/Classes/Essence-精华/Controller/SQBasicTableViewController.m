...
- (void)setupNotification{
    ...
    //监听tabBarButton的重复点击
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(tabBarButtonDidRepeatClick) name:SQTabBarButtonDidRepeatClickNotification object:nil];
    
    //监听titleViewButton的重复点击
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(titleButtonDidRepeatClick) name:SQTitleButtonDidRepeatClickNotification object:nil];
...
- (void)tabBarButtonDidRepeatClick{
    if (self.view.window == nil) {//由此过滤掉没有加载到window中的控制器
        return;}
    
    if(![self.view sq_intersectWithKeyWindow]){//由此过滤掉没有显示在window中的对象
        return;}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    commentVC.topicModel = self.groupOfModels[indexPath.row];
