...
- (void)leftNavbuttonClick{
    //创建推荐标签页面
    SQRecommondTagTableViewController *recommondTagTVC = [[SQRecommondTagTableViewController alloc] init];
    //利用Push方法弹出界面
    [self.navigationController pushViewController:recommondTagTVC animated:YES];    
}
...
- (void)setUpNavigation{
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem sq_itemWithImage:@"MainTagSubIcon" highImage:@"MainTagSubIconClick" target:self action:@selector(leftNavbuttonClick)];
}
...
- (void)titleViewButtonClick:(SQTitleViewButton *)titleViewButton{
    ...
    //如果重复点击了某个titleViewButton就发出通知
    if (titleViewButton == self.selectedButton) {
        [[NSNotificationCenter defaultCenter] postNotificationName:SQTitleButtonDidRepeatClickNotification object:nil];
    }
...
- (void)addChildTableView{
    ...
    //设定背景色
    tableView.backgroundColor = SQCommonBgColor;
    //设置tableView的内边距
