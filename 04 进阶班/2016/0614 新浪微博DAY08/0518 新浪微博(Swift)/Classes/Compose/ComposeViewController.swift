...
class ComposeViewController: UIViewController {
    ...
    @objc private func sendBtnClick() {
        ...
        if photoVC.groupOfImages.count > 0 {
            //是图片微博           
            NetWorkTools.shareInstance.sendStatus(textView.emoticonString(), image: photoVC.groupOfImages.first!, finished: { (dict, error) in
                if error != nil {
            ...
            //是文字微博
            NetWorkTools.shareInstance.sendStatus(textView.emoticonString()) { (dict, error) -> () in
                if error != nil {
                    SVProgressHUD.showErrorWithStatus("发送失败")
                    SVProgressHUD.setDefaultMaskType(.Black)
                    return
