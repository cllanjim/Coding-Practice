...
//    _cellHeight += textSize.height + SQCommonMargin;
    ...
    if (self.type != SQTopicTypeWord) {//如果当前帖子是非纯文字帖时,需要加入中间部分的高度
        ...
        //保存中间控件的frame值
        self.contentFrame = CGRectMake(SQCommonMargin, _cellHeight, contentW , contentH);
//        NSLog(@"%@",NSStringFromCGRect(self.contentFrame));
    ...
    if (self.top_cmt) {//如果不为空,就代表有最热评论数据
        ...
        _cellHeight += 20;
        //最热评论模块的内容
        NSString *topComment = [NSString stringWithFormat:@"%@ : %@", self.top_cmt.user.username, self.top_cmt.content];
