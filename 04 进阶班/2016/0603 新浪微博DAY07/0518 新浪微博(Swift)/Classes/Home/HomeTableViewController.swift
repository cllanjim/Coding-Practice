...
class HomeTableViewController: BaseTableViewController {
    ...
    var popoverAnimationDelegate = PopoverAnimationDelegate()
    
    var photoBrowserAnimateDelegate = PhotoBrowserAnimateDelegate()
    ...
    func showPictureView(notice: NSNotification) {
        //1.获取发出通知的cell
        guard let pictureCollectionView = notice.object as? HomeCellPictureCollectionView else {
            return
        }
        ...
        photoBrowserAnimateDelegate.delegate = pictureCollectionView
        photoBrowserAnimateDelegate.index = index
        
        //设置转场的代理
        vc.transitioningDelegate = photoBrowserAnimateDelegate
        //设置转场的类型
    ...
    @objc private func titleBtnClick(btn: HomeTitleButton) {
        ...
        //3.设置转场代理
        vc.transitioningDelegate = popoverAnimationDelegate
        vc.modalPresentationStyle = .Custom
        //4.弹出界面
        presentViewController(vc, animated: true, completion: nil)
    }
