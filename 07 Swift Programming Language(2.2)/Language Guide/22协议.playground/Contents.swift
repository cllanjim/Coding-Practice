...
struct Person0: FullyNamed {
    var fullName: String
}
let john = Person0(fullName: "John Appleseed")
 ...
 * 可失败构造器要求
 */
//协议还可以为采纳协议的类型定义可失败构造器要求，详见可失败构造器。
...
//DiceGame 协议可以被任意涉及骰子的游戏采纳。
//DiceGameDelegate 协议可以被任意类型采纳，用来追踪 DiceGame 的游戏过程。
 ...
 * 通过扩展添加协议一致性
 */
//即便无法修改源代码，依然可以通过扩展令已有类型采纳并符合协议。扩展可以为已有类型添加属性、方法、下标以及构造器，因此可以符合协议中的相应要求。详情请在扩展章节中查看。
...
//比如，一个类型为 (Int) -> String 的方法会变成 ((Int) -> String)?。需要注意的是整个函数类型是可选的，而不是函数的返回值。
...
//该特性表示协议将暴露给 Objective-C 代码，详情参见Using Swift with Cocoa and Objective-C(Swift 2.2)。即使你不打算和 Objective-C 有什么交互，如果你想要指定可选的协议要求，那么还是要为协议加上 @obj 特性。
